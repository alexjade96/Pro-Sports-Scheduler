...
- **HC8** Back-to-back ceiling (≤16/team) — soft-enforced via SC1 (target 14); a season-global hard cap makes the 221K-variable model intractable, so the validator reports overages as soft
...
│   ├── validator.py               # Post-solve validator — dispatches to the active league's rules
│   └── leagues/{nfl,nba}/         # Per-league validators (EPL is inline in validator.py)
│
...
- **The web dashboard and `tools/export_analytics.py` / `tools/solver_accuracy_viz.py` support all three leagues** via a `?league=` query param / `--league` flag. EPL keeps its Atos-Golden-Rule-flavored charts (Boxing Day coverage, SC13 pattern violations, London cluster) unchanged; NFL and NBA substitute their own signature metrics (Thanksgiving coverage/primetime share; back-to-backs/4-in-5/All-Star-break compliance) for the panels that depend on EPL-only `MetricsReport` fields — the web dashboard's `/analysis` page shows a banner explaining this rather than presenting EPL-only fields as zeros. Hard/soft-violation counts are now populated for all three leagues by the per-league validators.
- **`core/validator.py` validates all three leagues via a dispatcher** — `validate(schedule, teams, league=None)` delegates to `core/leagues/<league>/validator.py` for NFL/NBA (EPL inline), each returning the same report shape. The NFL/NBA validators check schedule-verifiable constraints (game counts, shared-venue/Christmas conflicts, Thursday rest, back-to-backs, 4-in-5, All-Star break, consecutive runs, …); generator-guaranteed structural rules and those needing external data (travel miles, arena windows, broadcast slots) are documented but not re-checked. On each league's CP-SAT output all validate with **0 hard violations** — every hard-modellable rule is enforced. The one exception is NBA's HC8 back-to-back ceiling, which is soft-enforced via SC1 because a season-global hard cap makes the model intractable (see the NBA constraints note above).
...
- **EPL and NBA historical CSVs are fetched from public GitHub mirrors, not their primary sources**, both of which the sandbox proxy blocks. EPL's 10 real seasons (2015-16 through 2024-25) come from the openfootball public-domain dataset via `data/leagues/epl/historical/download_seasons.py`, written back out in football-data.co.uk CSV format so the loader is unchanged. (These were synthetic placeholder files until they were replaced with the real fixtures; the synthetic data scrambled match dates and distorted every date-derived historical baseline — rest, consecutive runs, festive coverage, SC13 — so accuracy comparisons made against it were unreliable.) `generate_synthetic.py` stays as a fallback generator.
