...
NAME_MAP_FILENAME = "team_name_map.json"


def _load_name_map(historical_dir: Path) -> dict[str, str]:
    with open(historical_dir / NAME_MAP_FILENAME) as f:
        raw = json.load(f)
...
def load_season(csv_path: Path, season_label: str | None = None) -> Schedule:
    ...
    name_map = _load_name_map(csv_path.parent)
