...
### Custom / experimental constraint add-ons

`solvers/custom_constraints.py` is a league-agnostic framework for prototyping *additional* scheduling constraints that are not (yet) in any league's core `constraints.json` / solver constraint set — the data-dependent rules that show as "❌ needs travel-distance / timezone data" in `tools/constraint_report.py`, and any other idea worth testing. It is deliberately kept **out** of the core solve path: nothing in `runner.py`/`registry.py`/the constraint sets depends on it unless a caller explicitly opts in.

- **Register** a constraint with the `@register(id, description, kind, default_weight, requires=(...), source=...)` decorator. The function receives `(schedule, teams, context, weight)` and returns `(penalty, violations, metric)`. `requires=("geo",)` marks a data dependency so it is reported as *skipped* (not silently 0) when the league has no geo file. `context` is built once by `build_context()` and carries the geo lookup plus `distance_miles` / `tz_gap` helpers (`haversine_miles` is the great-circle primitive).
- **Built-in examples**: `back_to_back_travel` (hard, prototypes NBA HC7 — >1000 mi on a b2b second night), `timezone_shift` (soft, NBA SC9 — >2 time zones on a b2b), `travel_fairness` (soft, NFL SC7 / general — season travel-mile imbalance), and `rest_disparity` (soft, NBA SC8 / general — opponent-rest inequality; **needs no external data**, so it runs for any league and demonstrates a data-free add-on).
- **Geo data** lives in `data/leagues/<league>/geo.json` (`{team_id: {lat, lon, tz, tz_offset}}`). Only NBA ships one today (approximate arena/city coordinates + nominal standard-time UTC offsets — DST intentionally ignored so time-zone-boundary counts are stable). `load_geo()` returns `{}` for a league without one, and geo-requiring constraints skip gracefully (EPL/NFL currently have no geo file).
- **Two ways to use one** (see the module docstring): (1) **Evaluate** against an already-solved schedule via `evaluate(schedule, teams, ids=...)` — measure whether the candidate constraint binds and how badly, without re-solving; (2) **Optimize** by folding it into the metaheuristic objective via `CustomAugmentedMHConstraintSet` (wraps any league's MH set, delegates `pre_assign`/`greedy_params`, adds the custom penalty to `score()`). MH-only because it scores whole `Schedule` objects each iteration, so any Schedule→penalty function drops in without touching a MIP model — a custom CP-SAT/ILP term must instead be expressed in model variables inside that league's constraint set. The SA loop calls `score()` thousands of times, so a whole-season sweep per call adds real per-iteration cost; fine for prototyping, but profile before treating a custom-augmented run as a like-for-like time-budget comparison.
- **CLI**: `tools/custom_constraints.py` — `--list` (show registered constraints), `--league nba [--csv ...] [--constraints id,id]` (evaluate against a solved CSV; defaults to the league's CP-SAT output), or `--solve --time-limit N` (re-solve with the metaheuristic optimizing for the selected add-ons and print a before/after custom report). `run_solver(..., custom_constraint_ids=[...])` is the programmatic opt-in (raises for non-MH solvers).
