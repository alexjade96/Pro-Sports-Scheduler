"""
EPL constraint set for the ILP (PuLP) solver.

Wraps ``solvers/ilp/constraints.py`` with zero behaviour changes.
"""
...
class EPLILPConstraintSet:
    ...
    def add_soft_constraints(self, prob, x, fixtures, slots, teams) -> list:
        ...
        terms += add_soft_half_season_balance(
            prob, x, fixtures, slots, teams,
            tolerance=sc5.get("tolerance", 2), penalty=sc5.get("penalty_per_violation", 15),
        )
        sc14 = s.get("SC14", {})
        terms += add_soft_sc14_season_boundary(
            prob, x, fixtures, slots, teams, penalty=sc14.get("penalty_per_violation", 30),
        )
        sc15 = s.get("SC15", {})
        terms += add_soft_sc15_boxing_day_nyd(
            prob, x, fixtures, slots, teams, penalty=sc15.get("penalty_per_violation", 35),
        )
        sc17 = s.get("SC17", {})
